// task hunger
task Hunger[900000]() // 15 phut
{
	foreach(new i: Player)
	{
		new string[1280];
		new randdoibung = Random(10, 27);
		new randkhatnuoc = Random(10, 23);
		if(PlayerInfo[i][pFood] >= 20 && PlayerInfo[i][pThirsty] >= 20)
		{
			PlayerInfo[i][pFood] -= randdoibung;
			PlayerInfo[i][pThirsty] -= randkhatnuoc;
			UpdateHungerBar(i);
			format(string, sizeof(string), "{FF9933}Hunger{FFFFFF}: Ban da bi tru {FFFF00}%i% {FFFFFF}the luc va {FFFF00}%i%{FFFFFF} nuoc.", randdoibung, randkhatnuoc);
			SendClientMessageEx(i, COLOR_WHITE, string);
			return 1;
		}
		else {
			SetPlayerHealth(i, 0.0);
			SendClientMessageEx(i, COLOR_WHITE, "{FF9933}Hunger{FFFFFF}: Ban bi {FFFF00}Ngat xiu {FFFFFF}vi qua doi.");
			return 1;
		}
	}
	return 1;
...
task ProductionUpdate[300000]()
{
	...
	foreach(new i: Player)
	{
		...
		if(PlayerInfo[i][pConnectHours] < 2) {
			SendClientMessageEx(i, COLOR_LIGHTRED, "Vu khi se bi gioi han 2 gio choi dau cho nguoi moi tham gia cong dong GTAV.NET.");
		}
